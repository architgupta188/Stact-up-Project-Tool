...
chatRouter.post('/report/:reportId/chat', requireAuth, chatLimiter, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
chatRouter.get('/report/:reportId/chat', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const reportId = getParam(req.params.reportId);
    const userId = req.user!.id;

    const [conversation] = await db.select().from(conversations).where(
      and(eq(conversations.reportId, reportId), eq(conversations.userId, userId))
    ).limit(1);
